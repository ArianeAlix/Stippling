...
public:
		...
		InputIterator end)
	{
		...
		std::list<Point> points; // store centroids
	...
	template <class InputIterator>
	std::vector<double> area_lloyd(InputIterator begin, // inside triangles
		InputIterator end)
	{
		if (Dt2::dimension() != 2) {
		...
		std::vector<double> areas; // store areas
			...
			v++)
		{
			...
			FT sum_areas = 0.0;
			for (InputIterator it = triangles.begin(); it != triangles.end(); it++)
			{
				Triangle& triangle = *it;
				const FT area = std::fabs(CGAL::to_double(triangle.area()));
				sum_areas += area;
	...
	//return a map of the pixels for each generator
	std::map<Point, std::vector<Point>> by_gen(int h, int w) {
		std::map<Point, std::vector<Point>> cells;

		if (Dt2::dimension() != 2) {
			std::map<Point, std::vector<Point>> map_error;
			std::vector<Point > vec_vide = {};
			map_error.insert(std::pair<Point, std::vector<Point>>(Point(-1, -1), vec_vide));
			//return map_error;
		}
